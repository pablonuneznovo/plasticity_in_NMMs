...
The code is organized into two directories, one with MATLAB scripts and one with Python scripts. The Python folder has an additional script to generate the Schirner et al. E/I balance tuning algorithm using numba to greatly speed up the processing time and therefore requires numba to be installed to run.
...
- Gallinaro et al. (Mean-field Homeostatic Structural Plasticity): Population-level structural rewiring mechanism in which the calcium/activity trace regulates the creation and deletion of axonal and dendritic synaptic elements, changing the average connectivity between neuronal populations. 
